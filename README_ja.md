...
*コマンド1つで、10体のAIエージェントが並列稼働 — **Claude Code / OpenAI Codex / GitHub Copilot / Kimi Code / Gemini CLI** 混成軍*
...
| **Multi-CLI** | Claude Codeのみ | Claude Codeのみ | 任意のLLM API | 任意のLLM API | **5 CLI**（Claude/Codex/Copilot/Kimi/Gemini） |
...
| **Gemini CLI** | 長大なコンテキスト（1M+）、`GEMINI.md` 自動読み込み、無料プラン枠が充実 | Gemini 2.0 Flash / Pro |
...
↓ ビルド
CLAUDE.md / AGENTS.md / copilot-instructions.md / GEMINI.md  ← CLI別に生成
```
...
### Gemini CLI をメイン（将軍）として使用する設定例
Gemini CLI を主軸とした軍団構成にするには、`config/settings.yaml` を以下のように設定します：

```yaml
cli:
  default: gemini  # 全エージェントのデフォルトを Gemini に設定
  agents:
    shogun:
      type: gemini
...
**Thinking制御**:
 `config/settings.yaml` でエージェントごとに `thinking: true/false` を設定可能。`thinking: false` の場合、`MAX_THINKING_TOKENS=0` で起動しExtended Thinkingを無効化。ペインボーダーにはThinking有効時に `+T` サフィックスが表示される（例: `Sonnet+T`、`Opus+T`）。
