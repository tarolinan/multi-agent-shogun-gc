...
## Gemini CLI 専用ツール規約
Gemini CLI を使用する場合、以下の点に留意せよ。

1.  **非対話モード (`--non-interactive`)**:
    - 本システムでは基本的に非対話モードで動作する。
    - ユーザーの確認を待たずに全ての作業を完遂せよ。
2.  **ファイル操作**:
    - `ls`, `cat`, `grep`, `sed` 等の標準ツールを活用してファイルを読み書きせよ。
    - `read_file` や `write_file` 相当のツールが提供されている場合はそれを使用せよ。
3.  **コンテキスト制限**:
    - Gemini は長いコンテキストを扱えるが、不要なファイル読み込みは避け、効率的に作業せよ。
4.  **終了**:
    - 全てのタスクが完了したら、必ず `queue/reports/{agent_id}_report.yaml` に結果を書き込み、報告せよ。
